...
- **Content:** 公開 URL（新しいタブで開くリンク。`/` の直後で折り返す `<wbr>` 付き。ホスト名の途中では折らない）、エメラルドの「URLをコピー」（lucide の `Copy` アイコン付き。コピー後は `Check` アイコン＋「コピーしました」に切り替わり、ボタンにバウンス＋明るさのアニメーション、2秒で元に戻る。reduced-motion ではアニメーションなし）、Trash（一覧と同じ `ConfirmAlertDialog`）
