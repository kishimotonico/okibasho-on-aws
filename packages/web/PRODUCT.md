...
- 一覧の「再アップロード」はフォームに slug を入れて入力へフォーカスし、フォームまでスクロールする。保存期間は常に表示する。既存 slug への差し替えは吹き出し確認。slug が一覧の既存ページと一致している間（この操作で入った場合も、手入力で一致した場合も）は公開URLの枠の右端に「再アップロード」の小さなラベルと × が出て、× で乱数 slug・保存期間・公開範囲・パスワードを初期状態に戻して新規アップロードに切り替えられる
- 一覧の各ページから ShareDialog を開き、外部共有 URL を発行・作り直し・停止できる。未発行なら「共有URLを発行」だけを表示する（この時点ではパスワードを付けない）。発行済みなら共有URLを常に表示し、パスワードのオン/オフ共通トグル（PasswordToggle）で付け外しできる。オンにした瞬間にシステムがパスワードを自動生成して保存し、`--bg` 地の小さなブロックにユーザー名（`guest`）とパスワードを読み取り表示し、それぞれ個別にコピーできる（「まとめてコピー」で3行まとめてコピーも可能。オフのときは URL 単体のコピーで足りるため出さない）。共有中のページは slug 横に地球儀のボタンも出し（パスワードを付けていれば GlobeLock、付けていなければ Globe）、押すと同じ ShareDialog が開く。「作り直す」（新しい共有URLを発行する。パスワードを付けている場合はそれも同時に作り直し、古いものは失効する）と「共有を停止」は確認ダイアログ付き。外部共有 URL は発行してから開けるようになるまで数秒のラグがあるため、発行・作り直しの直後は URL の直下に控えめな1行で「開けるようになるまで数秒かかることがあります」、停止の直後は同じ位置に「無効になるまで数秒かかることがあります」と案内する。保存期限があれば URL の直下に小さなメタ行（時計アイコン）で併記する
...
- 要件と設計のドキュメント: `docs/concept.md`（目的・MVP スコープ・完成イメージ）、`docs/architecture.md`（決定済み設計）、`docs/roadmap.md`（フェーズと受け入れ条件）
