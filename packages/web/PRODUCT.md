...
- `/`: 上にドロップ即公開（単一 HTML / フォルダ / ドラッグ＆ドロップ。白いウェル全体がドロップ領域。箱クリックでファイル選択、テキストリンクでフォルダ選択。送信 CTA なし）。公開URL の slug は初期表示から `generateRandomSlug`（小文字英数字 10 文字）が入る。保存期間（30 日 / 無期限）もウェル内。下にアップロード済みページ（ページを開く、公開URL のコピー、保存期間の変更、削除）
- ドロップまたはファイル選択で、その時点の slug と保存期間で即アップロード。進捗は HTML ファイル数のみ（metadata の Put は数えない。1 件なら「1件」）
- バリデーションエラー・アップロード失敗は箱の吹き出し。新規アップロードで既存 slug にぶつかったときだけ上書き確認（差し替える / やめる）。再アップロードは確認なし
- 成功後は箱の下に URL・コピー・開く・このページを消す（確認なし）。次のドロップまで残し、新規公開では slug を再生成
- 同じ slug への再アップロード（内容の差し替え。保存期間は変わらない。参照できるのは常に最新版）。一覧の「再アップロード」はフォームに slug をセットしてスクロールする。公開URL は読み取り専用になり、保存期間は出さない
- 一覧の期限表示は日本時間の絶対日時（例: `2026/8/20 21:00 まで（あと2日）`）。無期限は「無期限」。期限切れは `期限切れ（yyyy/M/d）`
- 一覧の削除と「30日に戻す」は Radix AlertDialog。「無期限に変更」はダイアログなしで即実行。`window.confirm` は使わない
...
3. 「今どこまで見せられているか」を隠さない。期限は一覧に日本時間の絶対日時で出す。削除と「30日に戻す」はダイアログで確認する。「無期限に変更」は即実行する。
