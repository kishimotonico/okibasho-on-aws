...
- 外部向け共有 URL は `https://pages.<service-domain>/s/<tag><share-id>/`（`tag` はページごとに決まる 11 文字、`share-id` はランダムな 22 文字）。ページ単位で発行し、ログイン不要。自動生成されたパスワードによる Basic 認証（ユーザー名は `guest` 固定）を常に伴う。IP アドレス制限（完全一致リスト）も仕組みとしては持つが、管理 UI には出さず metadata の直接編集でのみ設定する
...
- 保存期間の下に公開範囲（内部のみ / 外部にも公開）のセグメントがあり、既定は内部のみ。「外部にも公開」を選ぶと、パスワード保護つきの外部共有 URL を自動で発行する案内を表示するだけで、入力欄は持たない（ユーザー名・パスワードはシステムが自動生成し、あとで一覧の共有ダイアログから確認できる）。対象 slug が既に外部共有中のページなら、この選択は「外部共有中（設定はそのまま）」に固定され、差し替えても既存の外部共有設定を維持する
...
- 一覧の各ページから ShareDialog を開き、外部共有 URL を発行・作り直し・停止できる。未発行なら「共有URLを発行」だけを表示する。発行済みなら共有URL・ユーザー名（`guest`）・パスワードを常に読み取り表示し、それぞれコピーできる（「まとめてコピー」で3行まとめてコピーも可能）。共有中のページは slug 横に地球儀（GlobeLock）のボタンも出し、押すと同じ ShareDialog が開く。「作り直す」（新しい共有URLとパスワードを同時に発行し、古い URL とパスワードは失効する）と「共有を停止」は確認ダイアログ付き。保存後は「反映まで少し時間がかかることがあります」と、保存期間がある場合は期限も併記する。停止・作り直しの確認ダイアログでは URL やパスワードが使えなくなることを伝える
- パスワードは常にシステムが自動生成し、手入力や個別の変更はできない。「発行」または「作り直す」のたびに新しいパスワードが生成される。「最初の1回だけ表示」の完了画面は持たず、ダイアログを開けばいつでも同じ場所に平文のパスワードが表示・コピーできる
...
- アップロード前の設定で今回新しく「外部にも公開」を選んで公開できた場合は、結果ブロックとあわせて ShareDialog を自動で開く（発行済みの表示から始まり、共有URL・ユーザー名・パスワードがすでに見える）。「次のファイルを置く」で公開範囲は内部のみに戻る
