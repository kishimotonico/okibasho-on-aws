...
- 保存期間の下に公開範囲（内部のみ / 外部にも公開）のセグメントがあり、既定は内部のみ。「外部にも公開」を選ぶと、外部共有 URL を自動で発行する案内と「パスワードを付ける」のチェックボックス（既定はオフ）を表示する。オンにしたときだけシステムが自動生成したパスワード（ユーザー名は `guest` 固定）を付け、あとで一覧の共有ダイアログから確認・付け外しできる。対象 slug が既に外部共有中のページなら、この選択は「外部共有中（設定はそのまま）」に固定され、差し替えても既存の外部共有設定を維持する
...
- 一覧の各ページから ShareDialog を開き、外部共有 URL を発行・作り直し・停止できる。未発行なら「共有URLを発行」だけを表示する（この時点ではパスワードを付けない）。発行済みなら共有URLを常に表示し、「パスワードを付ける」のチェックボックスで付け外しできる。オンにした瞬間にシステムがパスワードを自動生成して保存し、ユーザー名（`guest`）とパスワードを読み取り表示・コピーできる（「まとめてコピー」で3行まとめてコピーも可能。オフのときは URL 単体のコピーで足りるため出さない）。共有中のページは slug 横に地球儀のボタンも出し（パスワードを付けていれば GlobeLock、付けていなければ Globe）、押すと同じ ShareDialog が開く。「作り直す」（新しい共有URLを発行する。パスワードを付けている場合はそれも同時に作り直し、古いものは失効する）と「共有を停止」は確認ダイアログ付き。保存後は「反映まで少し時間がかかることがあります」と、保存期間がある場合は期限も併記する
