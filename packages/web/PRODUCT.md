...
- 社内向け共有 URL は `https://pages.<service-domain>/p/<user>/<slug>/`。`<user>` はメールアドレスのローカル部（`@` より前）
- 社外向け共有 URL は `https://pages.<service-domain>/s/<share-id>/`。ページ単位で発行し、社内ログイン不要。任意でパスワード（Basic 認証）・IP アドレス制限を付けられる
...
- 一覧の各ページから開く ShareDialog に加えて、アップロード直後の結果ブロックにも「社外共有…」があり、同じダイアログを今アップロードしたページに対して開ける。ShareDialog の開閉と一覧の再取得は route 側で一元管理し、一覧から開いた場合と同じ経路で動く
