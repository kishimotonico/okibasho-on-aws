...
- 成功後は箱の下に URL（新しいタブで開くリンク）・エメラルドのコピー・Trash（一覧と同じ AlertDialog で削除確認）だけを表示し、slug・保存期間・ファイル選択・案内文は隠す。箱からは「公開しました」の吹き出しを出す。連続アップロードは受け付けず、成功状態の箱（「次のファイルを置く」の role="button"）にマウスを乗せると蓋が少し開きかけ、クリック（または Enter/Space）すると箱がヨー回転しながら開いてフォームの初期状態（乱数 slug・30日）に戻る。成功結果のゴミ箱または一覧でページを消したとき、その slug がフォームに残っていれば乱数に戻し、そのページの成功結果も初期状態に戻す
