...
フェーズ 1〜4 は実装済み。cleanup は別 Lambda を作らず、フェーズ 4 で定期処理に統合し、share projector は役割に合わせて PageMaintenance に改名した。フェーズ 0 の計測（変更前後の比較）と、同時実行 1 を外すかどうかの見直しはデプロイ後に行う（[roadmap.md](roadmap.md) の「デプロイ後に確認したい点」を参照）。
